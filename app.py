...
def build_aamva_stream(inputs, options):
    ...
    # --- 核心数据流顺序调整 ---
    body.append(f"DCF{inputs['dd_code'].upper()}\x0a") # 1. 鉴别码
    body.append(f"DCGUSA\x0a")
    
    if not options['hide_icn']: body.append(f"DCK{inputs['icn'].upper()}\x0a") # 2. ICN
    body.append(f"DCA{inputs['class'].upper()}\x0a") # 3. 等级
    if not options['hide_audit']: body.append(f"DCJ{inputs['audit'].upper()}\x0a") # 4. 审计码 (挪到DCA后)
...
def main():
    ...
    with st.sidebar:
        ...
        h_audit = st.checkbox("隐藏审计码 (DCJ)", False) # 默认显示审计码以便查看效果
        opts = {'hide_dah': h_dah, 'hide_height': h_h, 'hide_weight': h_w, 
                'hide_eyes': h_e, 'hide_hair': h_hair, 'hide_icn': h_icn, 'hide_audit': h_audit}
    ...
    with st.container(border=True):
        ...
        real_id = c2.toggle("REAL ID 标准 (DDA)", True)
        
        d_cols = st.columns(4)
        dob = d_cols[0].text_input("生日 (DBB)", "03/04/1969")
        iss = d_cols[1].text_input("签发日", "11/05/2023")
        exp = d_cols[2].text_input("过期日", "11/05/2027")
        rev = d_cols[3].text_input("修订日 (DDB)", "04/26/2022")
        
        # --- 本次核心修改：DCF -> DCK -> DCA -> DCJ ---
        admin_cols = st.columns([2, 3, 1, 2])
        dcf = admin_cols[0].text_input("鉴别码 (DCF)", "NONE")
        icn_input = admin_cols[1].text_input("ICN (DCK)", "66004729317182331201") if not h_icn else ""
        cl = admin_cols[2].text_input("等级 (DCA)", "D")
        audit_input = admin_cols[3].text_input("审计码 (DCJ)", "A020424988483") if not h_audit else ""

        i_cols = st.columns(2)
        rs = i_cols[0].text_input("限制 (DCB)", "NONE")
        ed = i_cols[1].text_input("背书 (DCD)", "NONE")
    ...
    if st.button("🚀 生成条码并分析数据流", type="primary", use_container_width=True):
        inputs = {
            ...
            'sex': "1", 'address': addr, 'dah': "", 'city': city, 'zip': zip_c, 
            'height': "070", 'weight': "181", 'eyes': "BLU", 'hair': "BRO", 
            'icn': icn_input, 'audit': audit_input, 'real_id': real_id, 'dd_code': dcf, 'class': cl, 'rest': rs, 'end': ed
        }
