...
def pdf417_generator_ui():
    ...
    st.caption("基于 AAMVA D20-2020 标准，使用**经过验证的美国 IIN 映射表**和**单文件 (Num Entries = 1)** 模式。")
    st.warning("⚠️ **结构异常:** Control Field 标识符被强制设置为 `DL03` (标准应为 `C03`)，Header Prefix 长度被修改为 17 字节以匹配您的测试结果。")
    ...
    if st.button("🚀 生成 PDF417 条码", type="primary"):
        ...
        with st.spinner("正在生成 AAMVA 数据并编码..."):
            try:
                ...
                # 注意：这里我们使用 find("DL03") 来检查用户要求的前缀
                c03_start_index = aamva_data.find("DL03") 
                
                if c03_start_index != -1:
                    # DL03 占 4 字节，长度部分从第 5 字节开始，共 5 字节
                    ...
                    st.error("⚠️ **结构错误:** 未能在数据流中找到 Control Field (DL03) 标识符。")
